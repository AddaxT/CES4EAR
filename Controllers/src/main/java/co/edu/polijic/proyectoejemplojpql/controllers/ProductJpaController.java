...
public class ProductJpaController implements Serializable {
    ...
    public List<ProductSales> getTopTenProducts(){
        EntityManager em = getEntityManager();
        TypedQuery<ProductSales> topTen = em.createQuery("SELECT NEW co.com.addax.jpql.dto.ProductSales(p, SUM(orders.quantity)) FROM Product p JOIN p.purchaseOrderList orders GROUP BY p ORDER BY SUM(orders.quantity) DESC", ProductSales.class);
    ...
    public List<ProductSales> getBottomTenProducts(){
        EntityManager em = getEntityManager();
        TypedQuery<ProductSales> bottomTen = em.createQuery("SELECT NEW co.com.addax.jpql.dto.ProductSales(p, SUM(orders.quantity)) FROM Product p LEFT JOIN p.purchaseOrderList orders GROUP BY p ORDER BY SUM(orders.quantity) ASC", ProductSales.class);
    ...
    public List<ProductSales> getTopIncomes(){
        EntityManager em = getEntityManager();
        TypedQuery<ProductSales> topIncomes = em.createQuery("SELECT NEW co.com.addax.jpql.dto.ProductSales(p, SUM(orders.quantity)) FROM Product p JOIN p.purchaseOrderList orders GROUP BY p HAVING SUM(p.purchaseCost*orders.quantity) > 50000d ORDER BY SUM(orders.quantity)", ProductSales.class);
